...
public class ProductPlacementDAO extends AbstractDAO<ProductPlacement> {
	...
	private final String ADD_PRODUCT_PLACEMENT = "INSERT INTO product_placement (cupboard_id, x_start, y_start, x_end, y_end, board_number) VALUES (?, ?, ?, ?, ?,?)";
	private final String UPDATE_PRODUCT_PLACEMENT_BY_INDEX = "UPDATE product_placement SET cupboard_id=?, x_start=?, y_start=?, x_end=?, y_end=?, board_number=? WHERE id=?";
	...
	public int insert(ProductPlacement productPlacement) throws SQLException {
		...
		statement.setInt(3, productPlacement.getyStart());
		statement.setInt(4, productPlacement.getxEnd());
		statement.setInt(5, productPlacement.getyEnd());
		statement.setInt(6, productPlacement.getBoardNumber());
